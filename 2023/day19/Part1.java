...
public class Part1 {
  public static void main(String[] args) {
    ...
    long res = 0;
    while (i < lines.size()) {
      // process part collections
      // start with in
      PartCollection pc = new PartCollection(lines.get(i++));
      String destination = workflows.get("in").processWorkflow(pc);
      while (!(destination.equals("A") || destination.equals("R"))) {
        destination = workflows.get(destination).processWorkflow(pc);
      }
